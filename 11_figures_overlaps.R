...
if (length(deg_data) > 0) {
    ...
    # Map DEG identifiers to F. graminearum (FGSG) orthologs so cross-species
    # overlaps are computed in a shared ID space (consistent with the rest of
    # the analysis). FOXG/FPSE genes without a KO-based ortholog are dropped.
    fox_ortho <- read_csv(file.path(RESOURCE_DIR, "fox_to_fgr_orthology.csv"), show_col_types = FALSE)
    fpu_ortho <- read_csv(file.path(RESOURCE_DIR, "fpu_to_fgr_orthology.csv"), show_col_types = FALSE)
    fox_v <- setNames(fox_ortho$FGSG_ID, fox_ortho$FOXG_ID)
    fpu_v <- setNames(fpu_ortho$FGSG_ID, fpu_ortho$FPSE_ID)
    org_code_v <- setNames(STUDY_INFO$org_code, as.character(STUDY_INFO$study_id))
    map_genes_to_fgsg <- function(genes, study_id) {
        oc <- org_code_v[[as.character(study_id)]]
        if (is.null(oc) || oc == "fgr") return(unique(genes))
        if (oc == "fox") return(unique(unname(fox_v[genes[genes %in% names(fox_v)]])))
        if (oc == "fpu") return(unique(unname(fpu_v[genes[genes %in% names(fpu_v)]])))
        unique(genes)
    }

    # Extract DEG sets per study (mapped to FGSG ortholog space)
    get_degs <- function(study_id, direction = "all") {
        ...
        map_genes_to_fgsg(unique(d$gene), study_id)
    }
